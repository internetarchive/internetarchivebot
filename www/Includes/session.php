...
class Session {
	...
	public function __construct() {
		...
		session_set_cookie_params( strtotime( $sessionLifeTime ) - time(), dirname( $_SERVER['SCRIPT_NAME'] ), $cookieParams["domain"], $sessionSecure, $sessionHttpOnly );
		setcookie( session_name(), session_id(), strtotime( $sessionLifeTime ), dirname( $_SERVER['SCRIPT_NAME'] ), $cookieParams["domain"], $sessionSecure, $sessionHttpOnly );
	}
	...
	public function destroy( $id ) {
		if( !isset( $this->delete_stmt ) ) {
			$this->delete_stmt = mysqli_prepare( $this->sessionDBObject, "DELETE FROM externallinks_sessions WHERE id = ?" );
		}
	...
	public function gc( $max ) {
		if( !isset( $this->gc_stmt ) ) {
			$this->gc_stmt = mysqli_prepare( $this->sessionDBObject, "DELETE FROM externallinks_sessions WHERE set_time < ?" );
		}
		$old = time() - $max;
	...
	private function getkey( $id ) {
		if( !isset( $this->key_stmt ) ) {
			$this->key_stmt =
				mysqli_prepare( $this->sessionDBObject, "SELECT session_key FROM externallinks_sessions WHERE id = ? LIMIT 1" );
		}
