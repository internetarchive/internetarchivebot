...
class XHProfRuns_Default implements iXHProfRuns {
	...
	private function checkTable() {
		if( !mysqli_query( $this->db, "CREATE TABLE IF NOT EXISTS " . DB::quoteIdentifier( SECONDARYDB ) . ".externallinks_profiledata (
	...
	public function get_run( $run_id, $type, &$run_desc ) {
		$sql =
			"SELECT * FROM " . DB::quoteIdentifier( SECONDARYDB ) . ".externallinks_profiledata WHERE `run_id` = " . mysqli_escape_string( $this->db, $run_id ) .
			";";
	...
	public function save_run( $xhprof_data, $type, $run_id = null ) {
		...
		if( !is_null( $run_id ) ) $sql = "REPLACE INTO " . DB::quoteIdentifier( SECONDARYDB ) . ".externallinks_profiledata (`type`, `profile_data`) VALUES ('" .
		...
		else $sql = "INSERT INTO " . DB::quoteIdentifier( SECONDARYDB ) . ".externallinks_profiledata (`type`, `profile_data`) VALUES ('" .
		            mysqli_escape_string( $this->db, $type ) . "', '" .
	...
	function list_runs() {
		...
		$sql = "SELECT `run_id`, `type`, `timestamp` FROM " . DB::quoteIdentifier( SECONDARYDB ) . ".externallinks_profiledata ORDER BY `run_id` DESC LIMIT 500;";
