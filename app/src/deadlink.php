...
if ( ( isset ( $locales[BOTLANGUAGE] )
	&& !in_array( $locale, $locales[BOTLANGUAGE] ) )
...
if ( file_exists( IAPROGRESS . "runfiles/" . WIKIPEDIA . UNIQUEID ) ) {
	$lastpage = unserialize( file_get_contents( IAPROGRESS . "runfiles/" . WIKIPEDIA . UNIQUEID ) );
}
...
while ( true ) {
	...
	do {
		...
		elseif ( $config['page_scan'] == 0 ) { //This fetches all the articles, or a batch of them.
			...
			elseif ( $iteration !== 1 || $pages === false ) {
				...
				$pages  = $pages[0];
				file_put_contents(
					IAPROGRESS . "runfiles/" . WIKIPEDIA . UNIQUEID . "c",
		...
		elseif ( $config['page_scan'] == 1 ) {  //This fetches only articles with a deadlink tag in it, or a batch of them
			...
			elseif ( $iteration !== 1 || $pages === false ) {
				...
				if ( $lastpage !== false ) {
					foreach ( $pages as $tcount => $tpage ) {
						if ( $lastpage['title'] == $tpage['title'] ) break;
					}
		...
		foreach ( $pages as $tid => $tpage ) {
			...
			if ( DEBUG === false || LIMITEDRUN === true ) {
				file_put_contents(
					IAPROGRESS . "runfiles/" . WIKIPEDIA . UNIQUEID . "stats",
					serialize(
	...
	if ( file_exists( IAPROGRESS . "runfiles/" . WIKIPEDIA . UNIQUEID . "stats" )
		&& LIMITEDRUN === false ) {
		unlink( IAPROGRESS . "runfiles/" . WIKIPEDIA . UNIQUEID . "stats" );
	}
