...
class DataGenerator {
				...
				if( $link['newdata']['archive_host'] == "webcite" ) {
					if( preg_match( '/\/\/(?:www\.)?webcitation.org\/(\S*?)\?(\S+)/i', $link['newdata']['archive_url'],
						...
						if( strlen( $match[1] ) === 9 ) {
							...
							$microepoch                     = $magicwords['microepoch'] = Utilities::to10( $match[1], 62 );
							$magicwords['epoch']            = floor( $microepoch / 1000000 );
							$magicwords['epochbase62']      = Utilities::toBase( floor( $microepoch / 1000000 ), 62 );
						} else {
							$magicwords['microepochbase62'] = Utilities::toBase( $match[1], 62 );
							$magicwords['microepoch']       = $match[1];
							$magicwords['epoch']            = floor( $magicwords['microepoch'] / 1000000 );
							$magicwords['epochbase62']      =
								Utilities::toBase( floor( $magicwords['microepoch'] / 1000000 ), 62 );
						}
					}
				} else {
					$magicwords['epoch']       = $link['newdata']['archive_time'];
					$magicwords['epochbase62'] = Utilities::toBase( $link['newdata']['archive_time'], 62 );
				}
	...
	public function generateNewCitationTemplate( &$link ) {
		...
		$magicwords['microepochbase62'] = Utilities::toBase( $link['newdata']['archive_time'] * 1000000, 62 );
