...
class svwikiParser extends Parser {
	...
	*/
	protected function retrieveDateFormat( $default = false ) {
		if( !is_bool( $default ) && preg_match( '/\d\d? (?:Januari|Februari|Mars|April|Maj|Juni|Juli|Augusti|September|Oktober|November|December) \d{4}/i', $default ) ) return 'j F Y';
		elseif( !is_bool( $default ) && preg_match( '/(?:Januari|Februari|Mars|April|Maj|Juni|Juli|Augusti|September|Oktober|November|December) \d\d?\, \d{4}/i', $default ) ) return 'F j, Y';
		else return 'o-m-d';
	}
	...
	*/
	protected function rescueLink( &$link, &$modifiedLinks, &$temp, $tid, $id ) {
		...
		if( $link['link_type'] == "link" || $link['link_type'] == "stray" ) {
			...
			if( trim( $link['link_string'], " []" ) == $link['url'] || $link['link_type'] == "stray" ) {
				...
				if( $link['link_type'] == "stray" && (isset( $link['archive_template']['parameters']['titel'] ) || isset( $link['archive_template']['parameters']['title'] )) ) {
					if( isset( $link['archive_template']['parameters']['titel'] ) ) $link['newdata']['link_template']['parameters']['titel'] = $link['archive_template']['parameters']['titel'];
					else $link['newdata']['link_template']['parameters']['titel'] = $link['archive_template']['parameters']['title'];
				}
                else $link['newdata']['link_template']['parameters']['titel'] = "Archived copy";
                //We need to define the access date.
				$link['newdata']['link_template']['parameters']['hämtdatum'] = date( $this->retrieveDateFormat( true ), $link['access_time'] );
	...
	protected function generateNewCitationTemplate(&$link, &$temp ) {
		$link['newdata']['archive_type'] = "parameter";
		//Set the archive URL
		if( isset( $link['link_template']['parameters']['accessdate'] ) ) $link['newdata']['link_template']['parameters']['archiveurl'] = $temp['archive_url'];
		else $link['newdata']['link_template']['parameters']['arkivurl'] = $temp['archive_url'];

		//Set the archive date
		if( !isset( $link['link_template']['parameters']['arkivdatum'] ) && !isset( $link['link_template']['parameters']['archivedate'] ) ) {
			if( isset( $link['link_template']['parameters']['hämtdatum'] ) ) $link['newdata']['link_template']['parameters']['arkivdatum'] = date( $this->retrieveDateFormat( $link['link_template']['parameters']['hämtdatum'] ), $temp['archive_time'] );
			...
			else $link['newdata']['link_template']['parameters']['arkivdatum'] = date( $this->retrieveDateFormat(), $temp['archive_time'] );
		}
		else {
			if( isset( $link['newdata']['link_template']['parameters']['arkivdatum'] ) ) $link['newdata']['link_template']['parameters']['arkivdatum'] = date( $this->retrieveDateFormat( $link['newdata']['link_template']['parameters']['arkivdatum'] ), $temp['archive_time'] );
			else $link['newdata']['link_template']['parameters']['archivedate']  = date( $this->retrieveDateFormat( $link['newdata']['link_template']['parameters']['archivedate'] ), $temp['archive_time'] );
		}
	}
	...
	*/
	protected function analyzeCitation( &$returnArray, &$params ) {
		...
		if( isset( $returnArray['link_template']['parameters']['website'] ) && !empty( $returnArray['link_template']['parameters']['website'] ) ) $returnArray['url'] = $this->filterText( $returnArray['link_template']['parameters']['website'] );
		else return false;
		...
		} elseif( isset( $returnArray['link_template']['parameters']['hämtdatum'] ) && !empty( $returnArray['link_template']['parameters']['hämtdatum'] ) ) {
			$time = strtotime( $returnArray['link_template']['parameters']['hämtdatum'] );
			if( $time === false ) $time = strtotime( API::resolveWikitext( $returnArray['link_template']['parameters']['hämtdatum'] ) );
			if( $time === false ) $time = "x";
			$returnArray['access_time'] = $time;
		...
		if( isset( $returnArray['link_template']['parameters']['arkivdatum'] ) && !empty( $returnArray['link_template']['parameters']['arkivurl'] ) ) $returnArray['archive_url'] = $this->filterText( $returnArray['link_template']['parameters']['arkivurl'] );
		if( (isset( $returnArray['link_template']['parameters']['archiveurl'] ) && !empty( $returnArray['link_template']['parameters']['archiveurl'] )) || (isset( $returnArray['link_template']['parameters']['arkivurl'] ) && !empty( $returnArray['link_template']['parameters']['arkivurl'] )) ) {
		...
		if( isset( $returnArray['link_template']['parameters']['arkivdatum'] ) && !empty( $returnArray['link_template']['parameters']['arkivdatum'] ) ) {
			$time = strtotime( $returnArray['link_template']['parameters']['arkivdatum'] );
			if( $time === false ) $time = strtotime( API::resolveWikitext( $returnArray['link_template']['parameters']['arkivdatum'] ) );
			if( $time === false ) $time = "x";
			$returnArray['archive_time'] = $time;
		}
		...
		if( $this->commObject->isArchive( $returnArray['url'], $returnArray ) ) {
			...
			if( !isset( $returnArray['link_template']['parameters']['accessdate'] ) && !isset( $returnArray['link_template']['parameters']['hämtdatum'] ) && $returnArray['access_time'] != "x" ) $returnArray['access_time'] = $returnArray['archive_time'];
			else {
				...
				elseif( isset( $returnArray['link_template']['parameters']['hämtdatum'] ) && !empty( $returnArray['link_template']['parameters']['hämtdatum'] ) && $returnArray['access_time'] != "x" ) $returnArray['access_time'] = strtotime( $returnArray['link_template']['parameters']['hämtdatum'] );
				else $returnArray['access_time'] = "x";
			}
		}
