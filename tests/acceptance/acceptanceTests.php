...
class acceptanceTests extends PHPUnit_Framework_TestCase {
  ...
  public function testPages() {
    ...
    $testPageContents = preg_replace('/archivedate\s*=\s*\d{4}-\d{2}-\d{2}|date\s*=\s*\d{14}/', 'PLACEHOLDER', $testPageContents);
    $testCaseContents = preg_replace('/archivedate\s*=\s*\d{4}-\d{2}-\d{2}|date\s*=\s*\d{14}/', 'PLACEHOLDER', $testCaseContents);
